...
def copy_data_to_redshift(s3_path, table_name, column_list):
    ...
    try:
        ...
        SET 
            ...
            Date_Added = COALESCE(
                TO_TIMESTAMP(NULLIF(staging.Date_Added, ''), 'MM/DD/YYYY HH12:MI:SS AM'),
                Lead.Date_Added
            ),

            Date_Modified = COALESCE(
                TO_TIMESTAMP(NULLIF(staging.Date_Modified, ''), 'MM/DD/YYYY HH12:MI:SS AM'),
                Lead.Date_Modified
            ),
            ...
            First_Contact_Attempt_Date = COALESCE(
                TO_TIMESTAMP(NULLIF(staging.First_Contact_Attempt_Date, ''), 'MM/DD/YYYY HH12:MI:SS AM'),
                Lead.First_Contact_Attempt_Date
            ),
            ...
            Last_Action_Date = COALESCE(
                TO_TIMESTAMP(NULLIF(staging.Last_Action_Date, ''), 'MM/DD/YYYY HH12:MI:SS AM'),
                Lead.Last_Action_Date
            ),

            First_Assignment_Distribution_Date = COALESCE(
                TO_TIMESTAMP(NULLIF(staging.First_Assignment_Distribution_Date, ''), 'MM/DD/YYYY HH12:MI:SS AM'),
                Lead.First_Assignment_Distribution_Date
            ),
            ...
            "TO_Date" = COALESCE(
                TO_TIMESTAMP(NULLIF(staging."TO_Date", ''), 'MM/DD/YYYY HH12:MI:SS AM'),
                Lead."TO_Date"
            ),
            ...
            Last_Activity = COALESCE(
                TO_TIMESTAMP(NULLIF(staging.Last_Activity, ''), 'MM/DD/YYYY HH12:MI:SS AM'),
                Lead.Last_Activity
            ),
        ...
        WHERE Lead.Lead_ID = NULLIF(staging.Lead_ID, '')::INT;
        """
        ...
        INSERT INTO public.Lead (
            ...
            Date_Added, Date_Modified, Last_Action, First_Contact_Attempt_Date,
            Action_Count, Total_Contact_Attempts, Last_Action_Date,
            First_Assignment_Distribution_Date, First_Assignment_Distribution_User,
            Lead_Source_Group, Creative, Broker, Opener,
            IRA_Investment_Dollar, Cash_Investment_Dollar,
            Deal_Type, Transfer_Type, "TO_Date",
            SF_Lead_ID, Velocify_ID, Original_Broker, SF_Lead_Owner,
            Junior_Broker, Last_Activity, Intellect_Client_ID,
            Intellect_Broker, First_Name, Last_Name,
            Home_Phone, Work_Phone, Mobile_Phone,
            Email, Secondary_Email, Address, City, State,
            Zip_Postal_Code, Source_Code, SubID
        )

        SELECT DISTINCT
            NULLIF(staging.Lead_ID, '')::INT,
            staging.Source,
            staging.Lead_Status,
            staging.Lead_Score,
            staging.Milestone,
            ...
            NULLIF(staging.Action_Count, '')::INT,
            NULLIF(staging.Total_Contact_Attempts, '')::INT,
        ...
        FROM public.staging_lead staging
        WHERE NULLIF(staging.Lead_ID, '')::INT IS NOT NULL
        AND NOT EXISTS (
            SELECT 1
            FROM public.Lead l
            WHERE l.Lead_ID = NULLIF(staging.Lead_ID, '')::INT
        );

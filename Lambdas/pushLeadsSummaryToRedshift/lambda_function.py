...
def lambda_handler(event, context):
    ...
    for row in reader:
        ...
        try:
            if row[5].strip():
                row[5] = int(float(row[5]))  # ✅ Convert float -> int
            else:
                row[5] = ''
        except ValueError:
            row[5] = ''
    ...
    copy_sql = f"""
        COPY public.leads(lead_summary_id,lead_id,lead_name, status,lead_score_by_broker,total_contact_attempts,lead_type,lead_type_reason)
        FROM 's3://sample540/processed/leads_summary.csv'
